...
export default function Home() {
    ...
    const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const newImages = await Promise.all(Array.from(files).slice(0, 3).map(async (file) => {
    ...
    return (<div className="flex flex-col min-h-screen bg-background gap-4 p-4">
            <header
            ...
            <main className="container mx-auto flex flex-col md:flex-row gap-4">
                                ...
                                {images.map((imageSrc, index) => (
                                    <button key={index} onClick={() => handleThumbnailClick(imageSrc)}>
                                        <img
                                            ...
                                            className={`w-20 h-20 object-cover rounded-md cursor-pointer ${selectedImage === imageSrc ? "ring-2 ring-accent" : ""}`}
                                        />
                                    </button>))}
                                {images.length > 0 && (<Button variant="outline" size="sm" onClick={handleClearImages}>
                                        Clear Images
                                    </Button>)}
                            </div>
                        </CardContent>
                ...
                <section className="md:w-2/3 flex flex-col gap-4">
                    <Card>
                        ...
                        <CardContent>
                            {selectedImage ? (<div className="relative rounded-md overflow-hidden">
                                    ...
                                    <AlertDescription>Please upload and select an image to start
                                        editing.</AlertDescription>
                                </Alert>)}
                        </CardContent>
                    </Card>
                </section>
